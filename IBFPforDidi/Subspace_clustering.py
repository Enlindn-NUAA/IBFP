...
class SelfRepresentation(BaseEstimator, ClusterMixin):
    ...
    def _spectral_clustering(self):
        ...
        centroids, self.labels_, inertia = cluster.k_means(embedding, self.n_clusters,
                                                           random_state=random_state, n_init=self.n_init)
        print(centroids, 'centroids', np.array(centroids).shape)
        print(self.labels_, 'self.labels_')
        print(inertia, 'inertia')
        # print(best_n_iter,'best_n_iter')
        np.save('subspace-clusterend9.npy', self.labels_)
        # print(centroids,'centroids')
        np.save('subspace-clusterbasic9.npy', centroids)
        # print(clf.cluster_centers_,'cluster_centers_')
